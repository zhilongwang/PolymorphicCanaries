...
init_canary(void)
{
  canary canary1;
  canary canary2;
  const ssize_t filllen = sizeof(canary1.bytes);

  /*get random number as canarys*/
  ssize_t reslen1;
  ssize_t reslen2;
  ...
  if (fd >= 0) {
    reslen1 = read(fd, canary1.bytes, filllen);
    reslen2 = read(fd, canary2.bytes, filllen);
    if (unlikely(reslen1 != filllen || reslen2 != filllen)) {
  ...
  canary1.num = canary1.num & 0xFFFFFFFFFFFFFF00;
  canary check_canary;
  check_canary.num = canary1.num ^ canary2.num;
  //printf("%llu\n", check_canary.num);

  /* update the value in the TLS */
  SET_CANARY1(canary1.num);
  SET_CANARY2(canary2.num);
  SET_CHECK_CANARY(check_canary.num); 
  //printf("Init canary!");
}
/* 
...
renew_canary(void)
{
  ...
  /*get an random number as canary1*/
  int fd; /* file descriptor for /dev/urandom */
  ssize_t reslen;
  canary1.num = 0;
  fd = open(URANDOM_PATH, O_RDONLY);
  if (fd >= 0) {
    reslen = read(fd, canary1.bytes, filllen);
  ...
  canary1.num = canary1.num & 0xFFFFFFFFFFFFFF00;
  uintptr_t check_canary; 
  GET_CHECK_CANARY(check_canary);

  uintptr_t canary2 = canary1.num ^ check_canary;
  SET_CANARY1(canary1.num);
  SET_CANARY2(canary2);
}
